...
def toggle_editMacroPad():
    try:
        ...
        if not proceso_corriendo("MacroPadConfig.exe"):
            ...
            log.info("MacroPadConfig abierto.")
            return
        ...
        if not hwnd:
            ...
            log.info("MacroPadConfig relanzado (ventana no encontrada).")
            return
...
def toggle_whatsapp():
    try:
        ...
        if _ventana_minimizada(hwnd):
            _restaurar_ventana(hwnd)
            log.info("WhatsApp restaurado.")
        else:
            _minimizar_ventana(hwnd)
            log.info("WhatsApp minimizado.")
...
def cargar_configuracion():
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        config_base = {
            "E916FF00": {"tipo": "script", "valor": "mutear_discord", "nombre": "0: Mutear Discord"},
            "E718FF00": {"tipo": "script", "valor": "toggle_steam", "nombre": "2: Toggle Steam"}
        }
        ...
        try:
            ...
            with serial.Serial(PUERTO_SERIAL, BAUDRATE, timeout=1) as arduino:
                ...
                while True:
                    ...
                    # Lectura NO-BLOQUEANTE: solo lee si hay bytes disponibles en el buffer
                    try:
                    ...
                    if bytes_disp > 0:
                        ...
                        while b"\n" in buffer_rx:
                            ...
                            if linea in ("PING", "IR_RESET", "WATCHDOG_RESET"):
                                continue

                            codigo = linea.upper()
                        

                            if len(codigo) > 10 or len(codigo) < 2:
                                continue
                    ...
                    if ahora - ultimo_contacto > TIMEOUT_ZOMBIE_SEG:
                        log.warning(f"Puerto congelado ({TIMEOUT_ZOMBIE_SEG}s sin actividad). Forzando reconexion...")
                        break
